...
function profileRoster(files:Row[],projectSlug:string){return files.map((file)=>{const value=record(file.frontmatter),activities=record(value.activityProfiles),id=text(value.id).replace(/^agent:/u,'');return {handle:`@${projectSlug}/${id}`,purpose:text(value.purpose),responsibilities:strings(value.responsibilities),capabilities:strings(value.capabilities),enabledActivities:Object.keys(activities).sort(),authoritySummary:{canWrite:Object.values(activities).some((profile)=>strings(record(record(record(profile).permissions).content).write).length>0)}};}).sort((left,right)=>left.handle.localeCompare(right.handle));}
function applicableObjective(file:Row){return text(record(file.frontmatter).status).toLowerCase()!=='draft';}
...
export async function readCoreContextPack(request:AgentExecutionRequest,input:{identity:{manifest:Row;sources:Row[]};focused:Row;message:Row}){
	...
	const projectRead=request.treeDx.readRepositories?.find((candidate)=>candidate.projectId===text(manifest.projectId));
	const current:LibrarySource={teamId:text(manifest.teamId),projectId:text(manifest.projectId),repositoryId:projectRead?.repositoryId??request.treeDx.repositoryId??'',baseRef:projectRead?.baseRef??request.treeDx.baseRef??'',source:'current-project'};
	...
	const applicable=[...projectObjectives.filter(applicableObjective).map((file)=>({file,source:current})),...teamObjectives.filter(applicableObjective).map((file)=>({file,source:team}))];
