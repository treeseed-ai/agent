...
describe('provider manager availability recovery', () => {
	it('refreshes the durable membership session sequence after process recreation', async () => {
		...
		try {
			const host = resolveProviderConfig({ env: { TREESEED_PROVIDER_DATA_DIR: root, TREESEED_PROVIDER_SOURCE_CLOSURE_DIGEST: 'closure-a', HOME: root } });
			const config: ProviderConnectionRuntimeContext = { ...host, connectionId: 'team-a', marketUrl: 'https://team-a.test', marketAudience: 'https://team-a.test', teamId: 'team-a', providerId: 'provider-a', membershipId: 'membership-a', accessToken: 'token-a', defaultExecutionProviderId: 'codex-sub' };
			...
			const availability = { offer: { capabilities: ['research'], maxConcurrentRunners: 1 }, executionProviders: [{ id: 'codex-sub', adapter: 'codex', nativeLimits: {}, capabilities: ['research'] }] };
			...
			expect(calls[0]?.body.executionProviders).toEqual([expect.objectContaining({ id: 'codex-sub', preferred: true })]);
			expect(calls[0]?.body.metadata).toMatchObject({ sourceClosureDigest: 'closure-a' });
			expect(calls[1]?.body.metadata).toMatchObject({ sourceClosureDigest: 'closure-a' });
		} finally {
