...
import { CapacityProviderCoordinator, type ProviderConnectionResult } from '../coordination/coordinator.ts';
import { ProviderGlobalSlotScheduler, providerManifestConcurrency } from '../coordination/connection-scheduler.ts';
import { loadProviderManifest } from '../configuration/manifest.ts';
import { ProviderLocalCapacityStore, type ProviderLocalSlotClaim } from '../capacity/capacity-core/local-capacity-store.ts';
...
export async function runMultiTeamProviderRunners(config: ProviderHostRuntimeConfig, options: {
	...
	const { runProviderRunnerOnce } = await import('../operations/runner-lifecycle.ts');
