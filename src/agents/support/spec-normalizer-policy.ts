...
function normalizeContentScope(
	...
	const invalidActions = (actions ?? []).filter((action) => !CONTENT_ACTIONS.includes(action as ContentAction));
	...
	return {
		models,
		actions: actions?.filter((action): action is ContentAction => CONTENT_ACTIONS.includes(action as ContentAction)),
