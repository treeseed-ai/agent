...
export async function prepareAssignmentSource(client: Pick<SandboxBrokerClient, 'sourceStatus' | 'source'>,
  ...
  const response = await request.authorizeSource(initial.recipientPublicKey);
  ...
  return { recipientPublicKey: initial.recipientPublicKey, authorization: current.authorization, authorize: request.authorizeSource };
}
