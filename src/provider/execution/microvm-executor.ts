...
const TOOL_PERMISSION:Record<string,string>={treedx_build_context:'treedx.build_context',treedx_read_files:'treedx.read_repository_files',treedx_search_files:'treedx.search_workspace',treedx_list_paths:'treedx.read_repository_files'};
...
export async function executeAssignmentTreeDxTool(request:Parameters<AgentExecutor['execute']>[0],tool:string,arguments_:Record<string,unknown>) {
	const metadata=object(request.assignment.metadata), policy=object(metadata.toolPolicy), allowed=Array.isArray(policy.allowed)?policy.allowed.map(String):[];
	...
	const selected=String(arguments_.project??arguments_.projectId??'').trim();const grant=selected?request.treeDx.readRepositories?.find((candidate)=>candidate.projectId===selected||candidate.projectSlug===selected):null;
...
export async function createMicrovmExecutor(config: ProviderHostRuntimeConfig, manifest: CapacityProviderManifestV5, adapter: V5Adapter): Promise<AgentExecutor> {
	...
	return {
		...
		async execute(request) {
			...
			const toolOutcomes: ReviewToolOutcome[] = [];
			...
			try {
				...
				await request.emit?.({ type: 'sandbox.created', occurredAt: new Date().toISOString(), summary: `Prepared Kata sandbox ${prepared.sandboxId}.`, payload: { sandboxId: prepared.sandboxId, profile: assignment.profile, guestImageDigest: assignment.guestImageDigest,
					...
					const toolPump=(async()=>{while(!toolsAbort.signal.aborted){const pending=await client.nextToolRequest(prepared.sandboxId,prepared.operationToken,toolsAbort.signal).catch((error)=>{if(toolsAbort.signal.aborted)return {request:null};throw error;});if(pending.request){try{const value=pending.request.tool==='treeseed_publish_review'?await review.publish(pending.request.arguments):await executeAssignmentTreeDxTool(request,pending.request.tool,pending.request.arguments);toolOutcomes.push(reviewToolOutcome(pending.request.tool));await client.completeToolRequest(prepared.sandboxId,prepared.operationToken,pending.request.id,{result:value},request.signal);}catch(error){toolOutcomes.push(reviewToolOutcome(pending.request.tool,error,[request.leaseToken,prepared.operationToken]));await client.completeToolRequest(prepared.sandboxId,prepared.operationToken,pending.request.id,{error:error instanceof Error?error.message:String(error)},request.signal);}if(toolOutcomes.length>32)toolOutcomes.shift();}else await new Promise((resolve)=>setTimeout(resolve,50));}})();
