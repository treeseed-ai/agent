...
import type { AgentVerificationAdapter } from '../../runtime/runtime-types.ts';
...
export function createVerificationAdapter() {
	const provider = String(
		process.env.TREESEED_AGENT_VERIFICATION_PROVIDER ?? getAgentProviderSelections().verification,
	).toLowerCase();
	if (provider !== 'local') {
