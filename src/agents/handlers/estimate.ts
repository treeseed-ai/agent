...
import { validateStructuredAgentEstimate, type StructuredAgentEstimate } from '@treeseed/sdk/agent-capacity';
...
function outputRequirements(value: unknown): StructuredAgentEstimate['expectedOutputs'] {
	if (!Array.isArray(value)) return [{ outputType: 'agent_estimate', required: true }];
	return value.map((entry) => {
		const record = readRecord(entry);
		return {
			id: readString(record?.id) ?? undefined,
			outputType: firstString(record?.outputType, record?.type) ?? 'agent_estimate',
...
function dependencySpecs(value: unknown): StructuredAgentEstimate['dependencies'] {
	if (!Array.isArray(value)) return [];
	return value.map((entry, index) => {
		const record = readRecord(entry) ?? {};
		return {
			id: readString(record.id) ?? `dependency-${index + 1}`,
			type: (readString(record.type) ?? 'capability') as StructuredAgentEstimate['dependencies'][number]['type'],
			requiredBefore: (readString(record.requiredBefore) ?? 'start') as StructuredAgentEstimate['dependencies'][number]['requiredBefore'],
			...
			humanInputPolicy: readRecord(record.humanInputPolicy) as StructuredAgentEstimate['dependencies'][number]['humanInputPolicy'],
			summary: readString(record.summary) ?? undefined,
		};
	});
}

function buildStructuredEstimate(context: Parameters<AgentHandler['emitOutputs']>[0], result: ExecutionContentResult): StructuredAgentEstimate {
	...
	const metadata = readRecord(result.snapshot.outputs?.structuredEstimate)
		?? readRecord(result.snapshot.outputs?.estimate)
		?? readRecord(result.snapshot.outputs?.metadata)
		?? {};
	const expectedCredits = Number(metadata.expectedCredits ?? metadata.credits ?? context.capacity?.envelope?.reservedCredits ?? 1);
	const minCredits = Number(metadata.minCredits ?? Math.max(0, expectedCredits));
	const maxCredits = Number(metadata.maxCredits ?? Math.max(minCredits, expectedCredits));
	return {
		id: readString(metadata.id) ?? `estimate-${context.runId}`,
		teamId: firstString(assignment?.teamId, context.capacity?.envelope?.teamId) ?? 'unknown-team',
		projectId: firstString(assignment?.projectId, context.capacity?.envelope?.projectId) ?? context.projectId,
		decisionId: firstString(input.decisionId, assignment?.decisionId, decisionInput?.decisionId) ?? null,
		...
		agentClass: firstString(metadata.agentClass, context.agent.agentClass, assignment?.projectAgentClassId) ?? context.agent.slug,
		agentId: context.agent.slug,
		minCredits,
		expectedCredits,
		maxCredits,
		confidence: (readString(metadata.confidence) ?? 'medium') as StructuredAgentEstimate['confidence'],
		riskLevel: (readString(metadata.riskLevel) ?? 'medium') as StructuredAgentEstimate['riskLevel'],
...
export const estimateHandler: AgentHandler = {
	...
	async emitOutputs(context, result) {
		...
		const status = validation.ok && output.status === 'completed' ? output.status : validation.ok ? output.status : 'waiting';
		return {
			...output,
			status,
			summary: validation.ok ? output.summary : `Estimate output is waiting for valid structured estimate fields: ${validation.diagnostics.map((entry) => entry.code).join(', ')}`,
			metadata: {
				...(output.metadata ?? {}),
