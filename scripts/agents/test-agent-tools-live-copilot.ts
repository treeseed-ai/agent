...
import { createManagedToolEnv, resolveToolBinary } from '@treeseed/sdk/managed-dependencies';
import { resolveLaunchEnvironment } from '@treeseed/sdk/workflow-support';
import { runCopilotTask } from '@treeseed/sdk/copilot';
...
async function runCopilotToolProof(input: {
	...
	const env = createManagedToolEnv(input.env);
...
const managedEnv = createManagedToolEnv(launchEnv);
const copilotBinary = resolveToolBinary('copilot', { env: managedEnv });
if (!copilotBinary) {
