...
function runPackedProviderRuntimeSmoke(installRoot: string) {
	...
	const providerEntrypoint = 'node_modules/@treeseed/agent/dist/provider/lifecycle/entrypoint.js';
	...
	run(process.execPath, [providerEntrypoint, 'plan', '--json'], installRoot, false, env);
	...
	run(process.execPath, ['--input-type=module', '-e', [
		...
		"  import('./node_modules/@treeseed/agent/dist/provider/configuration/config.js'),",
		"  import('./node_modules/@treeseed/agent/dist/provider/lifecycle/lifecycle.js'),",
		"  import('./node_modules/@treeseed/agent/dist/provider/operations/runner.js'),",
		"  import('./node_modules/@treeseed/agent/dist/services/runtime-paths.js'),",
		"  import('./node_modules/@treeseed/agent/dist/agents/support/registry.js'),",
			"]);",
...
function assertNoLocalDependencyLinks() {
	...
	for (const sectionName of ['dependencies', 'devDependencies', 'peerDependencies', 'optionalDependencies']) {
		for (const [dependencyName, version] of Object.entries(packageJson[sectionName] ?? {})) {
			if (version.startsWith('workspace:') || (version.startsWith('file:') && !isAllowedIsolatedTarballSpecifier(version))) {
	...
	for (const [entryKey, entryValue] of Object.entries(lockfile.packages ?? {})) {
		...
		if (
			resolved.startsWith('../')
			|| resolved.startsWith('./')
			|| (resolved.startsWith('file:') && !isAllowedIsolatedTarballSpecifier(resolved))
...
try {
	...
	run(process.execPath, ['node_modules/@treeseed/agent/dist/scripts/agents/agents.js', '--help'], installRoot);
